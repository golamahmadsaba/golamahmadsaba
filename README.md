<!--
**golamahmadsaba/golamahmadsaba** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
# Hi, I'm Golam Ahmad Saba 👋

🎓 Electrical & Electronic Engineering (EEE) Student at North South University  
💻 Tech Learner | Linux Enthusiast  
📍 Dhaka, Bangladesh  

---

## 🧠 About Me

I am an Electrical and Electronic Engineering student with a deep interest in technology, programming, and entrepreneurship. 

I believe in **discipline, consistency, and purposeful learning**.

Currently, I am improving my skills in **programming, Linux, and problem-solving**, with a long-term goal of becoming a **skilled engineer** who builds solutions that help people and create value in society.

---

## 🎯 My Current Focus

- Learning **Python** and **C++** for software development  
- Improving **Linux system skills**  
- Building strong **problem-solving ability**  
- Exploring **AI tools and automation**  
- Developing **discipline and daily learning habits**

---

## 🛠️ Skills & Tools

**Programming:**  
- C  
- Python (learning)  
- C++ (learning)

**Systems & Tools:**  
- Linux (Arch Linux)  
- Git & GitHub  
- Bash / Terminal  

**Engineering:**  
- Basic electronics  
- Circuit analysis  
- Embedded system concepts (learning)

---

## 🚀 Projects & Learning Journey

- Linux system customization & automation  
- Programming practice projects  
- Problem-solving using C and Python  
- Exploring real-world engineering applications  

> I believe small consistent steps build strong long-term success.

---

## 🌱 My Vision

To become:

- A **highly skilled engineer**
- A **problem solver who builds meaningful solutions**
- A person who helps others through **technology and leadership**

---

## 📫 Contact & Presence

- GitHub: https://github.com/golamahmadsaba  
- LinkedIn: https://www.linkedin.com/in/golamahmadsaba
- Email: golamahmadsaba@gmail.com

---

## 🔥 Personal Philosophy

> Discipline is stronger than motivation.  
> Consistency is stronger than talent.  
> Purpose is stronger than fear.

---

⭐ *I use GitHub not only to store code, but to document my growth journey.*
